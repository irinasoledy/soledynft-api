<template>
    <div>
        <div class="filter__option">
          <div class="filter__option-title">
            <span>Sort by</span>
            <span id="closeSort">&times;</span>
          </div>

          <div class="filter__check-container">
            <label class="check-container">
              Price Asc
              <input type="radio" name="statusSeller"  v-model="sortOptions.priceAsc" value="1"/>
              <span class="check-container__mark">
                <svg style="fill:currentColor" width="11" height="9" viewBox="0 0 11 9" xmlns="http://www.w3.org/2000/svg" > <path d="M3.64489 8.10164L0.158292 4.61504C-0.0511769 4.40557 -0.0511769 4.06594 0.158292 3.85645L0.916858 3.09786C1.12633 2.88837 1.46598 2.88837 1.67545 3.09786L4.02419 5.44658L9.05493 0.41586C9.2644 0.206391 9.60405 0.206391 9.81352 0.41586L10.5721 1.17445C10.7816 1.38392 10.7816 1.72355 10.5721 1.93303L4.40348 8.10166C4.19399 8.31113 3.85436 8.31113 3.64489 8.10164V8.10164Z" ></path> </svg>
              </span>
            </label>
          </div>

          <div class="filter__check-container">
            <label class="check-container">
              Price Desc
              <input type="radio" name="statusSeller" v-model="sortOptions.priceDesc" value="1"/>
              <span class="check-container__mark">
                <svg style="fill:currentColor" width="11" height="9" viewBox="0 0 11 9" xmlns="http://www.w3.org/2000/svg" > <path d="M3.64489 8.10164L0.158292 4.61504C-0.0511769 4.40557 -0.0511769 4.06594 0.158292 3.85645L0.916858 3.09786C1.12633 2.88837 1.46598 2.88837 1.67545 3.09786L4.02419 5.44658L9.05493 0.41586C9.2644 0.206391 9.60405 0.206391 9.81352 0.41586L10.5721 1.17445C10.7816 1.38392 10.7816 1.72355 10.5721 1.93303L4.40348 8.10166C4.19399 8.31113 3.85436 8.31113 3.64489 8.10164V8.10164Z" ></path> </svg>
              </span>
            </label>
          </div>

        </div>
        <div class="filter__footer">
          <button class="button" type="button" @click="sendOptions">
            Show Results
          </button>
        </div>
    </div>
</template>

<script>
    import { bus } from "../../app_mobile";

    export default {
        props: ["category", "site"],
        data() {
            return {
                sortOptions : {
                    priceAsc : false,
                    priceDesc : false,
                },
            };
        },
        mounted() {},
        methods: {
            sendOptions(){
                let sended = false
                for (const [key, value] of Object.entries(this.sortOptions)) {
                    if (value == 1) {
                        bus.$emit("shareSortOptions", key)
                        sended = true
                    }
                }
                if (sended == false) {
                    bus.$emit("shareSortOptions", 'default')
                }
            },
        },
    };
</script>
